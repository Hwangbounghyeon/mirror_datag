...
export const validateFiles = (
  ...
  if (duplicateFiles.length > 0) {
    return {
      isValid: false,
      error: ERROR_MESSAGES.DUPLICATE_FILES(duplicateFiles.map((f) => f.name)),
    };
  }
  ...
  // 5. ZIP 파일 체크
  if (zipFiles.length > 0 && (currentType === null || currentType === "zip")) {
  ...
  if (
    imageFiles.length > 0 &&
    (currentType === null || currentType === "images")
  ) {
