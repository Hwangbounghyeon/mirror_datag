...
const Step4 = ({ handleMove }: StepProps) => {
  ...
  const requesetCreateProject = async () => {
    ...
    try {
      ...
      if (!response.data) {
        setErrorMessage(response.error || "프로젝트 생성에 실패했습니다.");
      } else {
        router.push("/project");
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-[700px] w-full flex flex-col items-center justify-center">
      ...
      <section className="gap-2 grid grid-cols-2 md:grid-cols-4">
        <Card>
      ...
      <ButtonFooter
        ...
        nextButtonText={isLoading ? "생성중..." : "생성"}
        nextButtonFunction={undefined}
