...
const ScatterPlot = ({ data, selectedIndices, selectedPoints, onSelectPoints, displayMode, setDisplayMode, dragCallBack }: ScatterPlotProps) => {
  ...
  useEffect(() => {
    const dragSelectPlugin = {
      id: 'dragSelect',
      beforeEvent: function(chart: CustomChart, args: { event: { type: string; x: number; y: number; native: MouseEvent }}) {
        ...
        if (event.type === 'mousemove' && isDragging && dragStart && isDragMode && isMouseButtonDown) {
        ...
        if ((event.type === 'mouseup' || event.type === 'mouseout' || !isMouseButtonDown) && isDragging) {
