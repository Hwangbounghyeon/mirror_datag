...
import { HistoryData, ObjectDetectionLabels, ReductionResults } from '@/types/historyType';
...
const BarChart = ({ data, selectedLabels, setSelectedLabels, displayMode, onLabelsSelect }: BarChartProps) => {
    ...
    datasets: [
      {
        ...
        borderWidth: Object.keys(distributionData).map(
          label => selectedLabels.includes(label) ? 5 : 1
        ),
        borderColor: Object.keys(distributionData).map(
          label => selectedLabels.includes(label) 
            ? '#FF4500'
            : LABEL_COLORS[label] || 'rgba(0, 0, 0, 0.6)'
        ),
        hoverBackgroundColor: Object.keys(distributionData).map(
          label => selectedLabels.includes(label) 
            ? LABEL_COLORS[label] 
            : 'rgba(0, 0, 0, 0.6)'
        )
  ...
  const options: ChartOptions<'bar'> = {
    ...
    onClick: (event, elements) => {
      if (elements.length > 0) {
        ...
        const selectedPoints = data.results?.filter(result => {
          if (displayMode === 'prediction') {
            return newSelectedLabels.includes(result.predictions?.prediction || '');
          } else {
