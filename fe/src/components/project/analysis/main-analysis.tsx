...
export function MainAnalysis({ selectedHistory }: MainAnalysisProps) {
    ...
    const currentItems = selectedPoints.slice(
        indexOfFirstItem,
        indexOfLastItem
    );
    ...
    function extractFeatureData(data: HistoryData): DataPoint[] {
        ...
        return data.results
            .filter(
                (result) => result.features && Array.isArray(result.features)
            )
            .filter((result) => {
                if (displayMode === "real") {
                    return (
                        isObjectDetectionLabels(result.label) &&
                        result.label.label
                    );
                }
                return true;
        ...
        <div className="flex flex-col w-full min-w-[1000px] flex-grow bg-content1 mx-4 rounded-xl shadow-medium">
            {selectedHistory && selectedData ? (
                <div className="flex flex-col p-6 space-y-6">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow mb-4 w-full">
                        <div className="flex w-full gap-4">
                            <div className="w-full lg:w-1/2 h-[30rem]">
                                <LabelBarChart
                                    data={selectedData}
                                    selectedLabels={selectedLabels}
                                    setSelectedLabels={(s: string[]) =>
                                        setSelectedLabels(s)
                                    }
                            ...
                            <div className="w-full lg:w-1/2 h-[30rem]">
                                {selectedData.results ? (
                                    <PieChart
                                        ...
                                        setSelectedIndex={(n: number | null) =>
                                            setSelectedIndex(n)
                                        }
                                        onDataSelect={(points) => {
                                            setSelectedLabels([]);
                    ...
                    <div className="flex gap-6">
                        <div className="w-[60%] bg-white dark:bg-gray-800 rounded-xl shadow-sm pb-4 pt-2 px-4 border border-divider shrink-0">
                            ...
                            <div className="mt-4 flex gap-4">
                                <Select
                                    label="X축 Feature"
                                    selectedKeys={[
                                        selectedIndices.x.toString(),
                                    ]}
                                    onChange={(e) =>
                                        handleIndexChange("x", e.target.value)
                                    }
                                    classNames={{
                                ...
                                <Select
                                    label="Y축 Feature"
                                    selectedKeys={[
                                        selectedIndices.y.toString(),
                                    ]}
                                    onChange={(e) =>
                                        handleIndexChange("y", e.target.value)
                                    }
                                    classNames={{
                                        base: "flex-1 bg-default-100",
                                        trigger:
