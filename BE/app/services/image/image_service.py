...
class ImageService:    
    ...
    async def search_model_images_by_conditions(self, search_conditions: List[SearchCondition] | None, user_id: int, model_name: str | None = None, page: int = 1, limit: int = 10) -> PaginationDto[List[ImageSearchResponse]]:
        try:
            # 1. tag document 가져오기
            tag_doc = await self.collection_tag_images.find_one({})
            if not tag_doc:
